...
apply_deployment_change <- function(stations_folder_path, 
                                    string_tracking_sheet, 
                                    station_name, depl_date, 
                                    field_to_change, 
                                    old_value, 
                                    new_value, 
                                    rationale, 
  ...
  tracking_sheet_row <- c(station_name, 
                          depl_date, 
                          field_to_change, 
                          old_value, 
                          new_value, 
                          rationale, 
                          today_as_yyyy_mm_dd_string())
  # TODO: Add checks for old value to match new value in the case of station name 
  #and deployment date? Also that new value is not the same as old value? 
  #maybe also check for format?
  if (field_to_change == "station name") {
    completion_record <- apply_station_name_change(stations_folder_path, 
                                                   string_tracking_sheet, 
                                                   station_name, 
                                                   depl_date, 
                                                   new_value, 
    ...
    completion_record <- apply_deployment_date_change(stations_folder_path, 
                                                      station_name, 
                                                      depl_date, 
                                                      new_value, 
                                                      rationale, 
  ...
  tracking_sheet_row <- c(tracking_sheet_row, 
                          completion_record, 
                          todo_items, 
                          note)
  tracking_sheet_row_df <- as.data.frame(t(tracking_sheet_row), 
                                         stringsAsFactors=FALSE)
  sheet_append(change_tracking_sheet, 
               tracking_sheet_row_df, 
               sheet = "Water Quality")
}

apply_station_name_change <- function(stations_folder_path, 
                                      string_tracking_sheet, 
                                      old_station_name, 
                                      depl_date, 
                                      new_station_name, 
  ...
  old_station_folder_path <- get_relative_path_to_station_folder(rel_stations_folder_path, 
                                                                 old_station_name)
  new_station_folder_path <- get_relative_path_to_station_folder(rel_stations_folder_path, 
                                                                 new_station_name)
  old_deployment_folder_path <- get_relative_path_to_depl_folder(old_station_folder_path, 
                                                                 old_station_name, 
                                                                 depl_date)
  ...
  if (!check_station_folder_exists(new_station_folder_path)) {
    stop("Station folder structure does not exist. 
         Please run create_new_station() to create it.")
  }
  if (!check_station_area_info_exists(string_tracking_sheet, new_station_name)) {
    stop("Station area info does not exist. 
         Please run create_new_station() to create it.")
  }
  
  message("UPDATING FOLDER STRUCTURE")
  # Create new deployment folder - station folder should either have already 
  #existed or been created
  new_deployment_folder_path <- create_deployment_folder(rel_stations_folder_path, 
                                                         new_station_name, 
                                                         depl_date)
  
  # Copy content to new folders and delete old folders and contents
  are_deployment_files_copied <- copy_deployment_files(old_deployment_folder_path, 
                                                       new_deployment_folder_path)
  are_station_files_copied <- copy_station_files(old_station_folder_path, 
                                                 new_station_folder_path)
  are_deployment_files_deleted <- safe_delete_old_folder(old_deployment_folder_path, 
                                                         new_deployment_folder_path)
  is_depl_folder_updated <- (are_deployment_files_copied 
                             && are_station_files_copied 
                             && are_deployment_files_deleted)
  completion_record <- c(completion_record, is_depl_folder_updated)
  
  message("UPDATING STRING TRACKING SHEET")
  # Write old station name into old station name column, 
  #appending if there is already a value there
  is_old_name_archived <- update_string_tracking_column(string_tracking_sheet, 
                                                        old_station_name, 
                                                        depl_date, 
                                                        "old station name", 
                                                        old_station_name, 
  ...
  is_old_name_replaced <- update_string_tracking_column(string_tracking_sheet, 
                                                        old_station_name, 
                                                        depl_date, 
                                                        "station", 
                                                        new_station_name)
  completion_record <- c(completion_record, 
                         is_old_name_archived && is_old_name_replaced)
  
  message("UPDATING LOG CONTENTS AND NAME")
  # Update Log
  is_log_content_updated <- update_log_data(new_deployment_folder_path, 
                                            "Location_Description", 
                                            old_station_name, 
                                            new_station_name)
  # Update Log Name
  is_log_name_updated <- update_log_name(new_deployment_folder_path, 
                                         new_station_name, 
                                         depl_date)
  completion_record <- c(completion_record, 
                         is_log_content_updated && is_log_name_updated)
  
  message("UPDATING README IN THE STATION FOLDER")
  # Update README (at station folder level)
  readme_content <- build_readme_content("station name", 
                                         old_station_name, 
                                         new_station_name, 
                                         rationale, 
                                         note)
  is_readme_updated <- write_readme_file(new_station_folder_path, 
                                         readme_content)
  completion_record <- c(completion_record, is_readme_updated)
  
  message("UPDATING CONFIGURATION TABLE")
  # Update Config Table
  is_config_table_updated <- update_config_table_entry(old_station_name, 
                                                       "Station_Name", 
                                                       depl_date, 
                                                       new_station_name)
...
apply_deployment_date_change <- function(stations_folder_path, 
                                         station_name, 
                                         old_depl_date, 
                                         new_depl_date, 
                                         rationale, 
  ...
  message("Resolving current directory structure...")
  rel_stations_folder_path <- get_relative_path_from_wd_to_stations_folder(stations_folder_path)
  station_folder_path <- get_relative_path_to_station_folder(rel_stations_folder_path, 
                                                             station_name)
  old_deployment_folder_path <- get_relative_path_to_depl_folder(station_folder_path, 
                                                                 station_name, 
                                                                 old_depl_date)
  
  # Update Folder Structure
  message("UPDATING FOLDER STRUCTURE")
  # Create new deployment folder - station folder should either already exist 
  #or be created by station creation function
  new_deployment_folder_path <- create_deployment_folder(rel_stations_folder_path, 
                                                         station_name, 
                                                         new_depl_date)
  ...
  is_old_date_replaced <- update_string_tracking_column(string_tracking_sheet, 
                                                        station_name, 
                                                        old_depl_date, 
                                                        "deployment", 
                                                        new_depl_date)
  ...
  readme_content <- build_readme_content("deployment date", 
                                         old_depl_date, 
                                         new_depl_date, 
                                         rationale, 
                                         note)
  ...
  message("UPDATING CONFIGURATION TABLE")
  # Update Config Table
  is_config_table_updated <- update_config_table_entry(
    station_name, 
    "Depl_Date", 
    old_depl_date, 
    new_depl_date)
...
check_station_area_info_exists <- function(ss, station_name) {
  ...
  area_info_cell_found <- find_area_info_cell(sheet_data, 
                                              station_name, 
                                              "station")
...
create_new_station <- function(stations_folder_path, 
                               ss, 
                               station_name, 
                               waterbody, 
                               county, 
  ...
  if (!find_area_info_cell(sheet_data, station_name, "station")) {
    new_area_info_row <- c(station_name, 
                           waterbody, 
                           county, 
                           latitude, 
                           longitude, 
                           note)
    new_area_info_row_df <- as.data.frame(t(new_area_info_row), 
                                          stringsAsFactors=FALSE)
    sheet_append(string_tracking_sheet, 
                 new_area_info_row_df, 
                 sheet="Area Info")
...
update_string_tracking_column <- function(ss, 
                                          station_name, 
                                          depl_date, 
                                          column_name, 
                                          new_data, 
  ...
  cell_id <- find_string_tracking_cell(sheet_data, 
                                       station_name, 
                                       depl_date, 
                                       column_name)
...
update_area_info_column <- function(ss, 
                                    station_name, 
                                    column_name, 
                                    new_data, 
                                    append=FALSE) {
...
update_config_table_entry <- function(station_name, 
                                      column_to_update, 
                                      deployment_date, 
                                      new_value) {
  ...
  cb_config_entry <- nrow(cb_config_data %>% 
                            filter(Station_Name == station_name 
                                   & Depl_Date == ymd(deployment_date))) == 1
  # Check for entry in configuration table
  config_table_entry <- nrow(config_data %>% 
                               filter(Station_Name == station_name 
                                      & Depl_Date == ymd(deployment_date))) == 1
  if (cb_config_entry) {
      ...
      mutate_at(.vars = vars(column_to_update), 
                ~ case_when(
                  (Station_Name == station_name 
                   & Depl_Date == ymd(deployment_date)) ~ new_value,
    ...
    message("No entry found in configuration table. Skipping update.\n 
            If deployment is from 2023 or earlier, please check manually.")
    return(FALSE)
  }
  return(TRUE)
}
...
write_readme_file <- function(file_path, content) {
  ...
  if (length(file_list) > 1) {
    print("Multiple README files found. 
          Please manually condense the files into one.")
    return(FALSE)
  } else {
    if (length(file_list == 1)) {
...
build_readme_content <- function(field_to_change, 
                                 old_value, 
                                 new_value, 
                                 rationale, 
                                 note) {
  dated_content <- glue("{today_as_yyyy_mm_dd_string()} update: 
                        changed {field_to_change} from {old_value} to {new_value}.\n 
                        Rationale: {rationale} \n
                        Additional Note: {note}")
  return(dated_content)
}

update_log_name <- function(deployment_folder_path, 
                            updated_station_name, 
                            updated_depl_date) {
  ...
  return(rename_log(log_file_path, 
                    log_file_name, 
                    updated_station_name, 
                    updated_depl_date))
}
...
update_log_data <- function(deployment_folder_path, 
                            column_to_update, 
                            old_value, 
                            new_value) {
  ...
  # TODO: Check that the log value is in fact the provided old value 
  #to make sure we are looking at the right column?
