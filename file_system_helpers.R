...
today_as_yyyy_mm_dd_string <- function() {
  return(format(today(tzone="UTC"), format="%Y-%m-%d"))
}


# TODO: Replace this with file_ext from the fs package?
# File extension extraction function pulled from helpers-misc.R 
#in sensorstrings package on 2024-01-15
...
get_relative_path_to_station_folder <- function(rel_stations_folder_path, 
                                                station_name) {
...
get_relative_path_to_depl_folder <- function(rel_station_folder_path, 
                                             station_name, 
                                             depl_date) {
  snake_case_station_name <- to_snake_case(station_name)
...
# Template must be closer to the directory containing all the station folders 
#due to max path length limitations
# TODO: Add check to confirm this path exists - 
# this should also confirm that it is correctly calculated
...
strip_path_start <- function(full_path, path_start) {
  path_start_regex <- regex(paste0(path_start, "\\/([a-z0-9\\/_\\-]+)"), 
                            ignore_case=TRUE)
...
copy_deployment_files <- function(old_deployment_folder_path, 
  ...
  if (dir_exists(new_deployment_folder_path)) {
    ...
    stop("Deployment folder does not exist, though it should have been created. 
         Please check the file structure.")
  }
}

copy_station_files <- function(old_station_folder_path, 
  ...
  if (dir_exists(new_station_folder_path)) {
    ...
    stop("Station folder does not exist. 
         This should have been checked earlier in the code. 
         If you're encountering this error, please check the file structure.")
  }
}
...
safe_delete_old_folder <- function(old_folder_path, new_folder_path) {
  ...
  if (length(dir_diff) != 0) {
    message("Not all files were copied successfully or in the correct structure. 
            The following files are not in both folders {dir_diff}")
    return(FALSE)
  } else {
...
rename_log <- function(old_log_file_path, 
                       old_log_file_name, 
...
archive_log <- function(log_file_path, log_file_name) {
  ...
  file_name_regex <- regex("log\\/([a-z0-9_\\- ]+\\.csv$|[a-z0-9_\\- ]+\\.xlsx$|[a-z0-9_\\- ]+\\.xls$)",
                           ignore_case = TRUE)
  archive_log_file_name_local <- str_extract(log_file_name, file_name_regex)
...
# TODO: Reformulate this with trycatch? 
#Could lead to confusing errors if path passed to function is not valid?
# if condition to make sure we don't get a warning for trying to create it when it exists
create_deployment_folder <- function(station_folders_path, 
                                     station_name, 
                                     deployment_date) {
  snake_case_station_name <- to_snake_case(station_name)
  ...
  if (dir_exists(station_folders_path) && dir_exists(station_folder_path)) {
    ...
    stop("This path to the stations folder and/or the station folder do not exist. 
